...
export function QuoteDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchQuotes = async () => {
      try {
        const data = await (blink.db as any).quotes.list({
        ...
        if (data && data.length > 0) {
          ...
          const initialQuote = { ...DEFAULT_QUOTE }
          const newQuote = await (blink.db as any).quotes.create({
            ...initialQuote,
          ...
          if (newQuote) {
             setActiveQuotes([{ ...initialQuote, id: newQuote.id }])
          }
        }
      } catch (error) {
  ...
  const debouncedSave = useCallback(
    debounce(async (quote: QuoteData) => {
      ...
      setIsSaving(true)
      try {
        await (blink.db as any).quotes.update(quote.id, {
  ...
  const handleAddCard = async () => {
    ...
    try {
      ...
      if (newQuote) {
        setActiveQuotes([...activeQuotes, { ...newQuoteData, id: newQuote.id }])
  ...
  const handleRemoveCard = async (id: string) => {
    ...
    try {
      await (blink.db as any).quotes.delete(id)
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-[400px] h-full overflow-y-auto border-r border-slate-200 bg-white shadow-sm p-6 shrink-0 custom-scrollbar">
            ...
            <QuoteEditor 
              ...
              onSave={() => toast.success('Changes saved locally')}
              onChange={(updated) => handleQuoteChange(parseInt(activeTab), updated)}
            />
          </div>

          {/* Preview Canvas */}
          <div className="flex-1 h-full overflow-auto bg-slate-50 p-12 custom-scrollbar">
            <div ref={exportRef} className="mx-auto min-w-fit max-w-full p-8 bg-slate-50 rounded-2xl">
              <div className="flex flex-col gap-12">
                {/* Package Cards View */}
                <div className="flex flex-wrap justify-center gap-10">
                  {activeQuotes.map((q, idx) => (
                    <div key={q.id} className="w-[600px] shrink-0 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
                      ...
                      <table className="w-full border-collapse">
                        <thead>
                          <tr>
                            ...
                            {activeQuotes.map((q) => (
                              <th key={q.id} className="text-center py-4 px-6 bg-slate-50 border-b border-slate-200 text-slate-900 font-bold min-w-[200px]">
                                {q.packageName}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody className="divide-y divide-slate-100">
                          <tr>
                            ...
                            {activeQuotes.map((q) => (
                              <td key={q.id} className="py-4 px-6 text-center font-bold text-accent text-lg">
                          ...
                          <tr>
                            ...
                            {activeQuotes.map((q) => (
                              <td key={q.id} className="py-4 px-6 text-center">
                                <div className="flex flex-wrap justify-center gap-1">
                                  {q.platforms.map(p => (
                                    <span key={p} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full text-[10px] font-bold uppercase">{p}</span>
                                  ))}
                                </div>
                              </td>
                            ))}
                          ...
                          <tr>
                            ...
                            {activeQuotes.map((q) => (
                              <td key={q.id} className="py-4 px-6 text-center text-emerald-500">Included</td>
                            ))}
                          </tr>
                          {ADD_ONS.map((addon) => (
                            <tr key={addon}>
                              ...
                              {activeQuotes.map((q) => (
                                <td key={q.id} className="py-4 px-6 text-center">
                                  {q.addOns.includes(addon) ? (
                                    <span className="w-6 h-6 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto">✓</span>
                                  ) : (
                          ...
                          <tr className="bg-slate-50/50">
                            ...
                            {activeQuotes.map((q) => (
                              <td key={q.id} className="py-6 px-6 text-center font-black text-2xl text-accent">£{q.price}</td>
                            ))}
                          </tr>
                        </tbody>
                      </table>
