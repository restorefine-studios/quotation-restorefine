    ...
    <div className="space-y-6 animate-fade-in pb-20">
            ...
            >
              <div
                className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all ${
                  quote.addOns.includes(addon) ? 'bg-red-600 text-white scale-110' : 'bg-slate-100 text-slate-400'
                }`}
              >
                {quote.addOns.includes(addon) && <CheckCircle2 size={14} />}
              </div>
              <span className={`text-sm font-bold ${quote.addOns.includes(addon) ? 'text-slate-900' : 'text-slate-600'}`}>
                {addon}
              </span>
            </div>
          ))}
      ...
      <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-2xl p-5 border border-slate-200/50 shadow-sm">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-slate-500 italic pl-1">Per month, billed in advance</p>
        </div>
      </div>
